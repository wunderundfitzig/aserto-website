...
// Size distribution ratios: 3 small : 2 medium : 1 big
const SIZE_RATIOS = [3, 2, 1]
const SIZE_RATIOS_TOTAL = SIZE_RATIOS.reduce((a, b) => a + b, 0)
const SIZE_RATIOS_CUMULATIVE = SIZE_RATIOS.reduce<number[]>(
  (acc, r) => [...acc, (acc[acc.length - 1] ?? 0) + r],
  [],
)
...
function setup(
  ...
  const dotCount = Math.max(
    Math.min(Math.round(width * height * DOTS_PER_PX), MAX_DOTS),
    MIN_DOTS,
  )

  const dots: DotNode[] = Array.from({ length: dotCount }, (_, i) => {
    const angle = Math.random() * 2 * Math.PI
    const sizeIndex = SIZE_RATIOS_CUMULATIVE.findIndex(
      (cum) => i < (cum / SIZE_RATIOS_TOTAL) * dotCount,
    )
...
function forceExcludeRects(
  ...
  function force() {
    ...
    for (const dot of nodes) {
      ...
      const isMobile = widthRef.current < 600
      if (isMobile ? dot.sizeIndex < 2 : dot.sizeIndex < 1) continue
